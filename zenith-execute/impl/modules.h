...
namespace ctx
{
    auto write_protected_address( std::uintptr_t address, void* buffer, SIZE_T size, bool Restore ) -> BOOLEAN
    {
        NTSTATUS Status = { STATUS_SUCCESS };

        auto Mdl = qtx_import(IoAllocateMdl)(reinterpret_cast<void*>(address), size, FALSE, FALSE, NULL);

        qtx_import(MmProbeAndLockPages)(Mdl, KernelMode, IoReadAccess);

        auto Mapping = qtx_import(MmMapLockedPagesSpecifyCache)(Mdl, KernelMode, MmNonCached, NULL, FALSE, NormalPagePriority);
