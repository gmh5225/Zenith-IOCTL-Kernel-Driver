...
namespace request
{
	_declspec(noinline) zenith::status get_module_base( invoke_data* request )
	{
		base_invoke data = { 0 };
		uintptr_t out = 0;
		...
		PEPROCESS process = 0;
		if ( qtx_import(PsLookupProcessByProcessId)( ( HANDLE ) data.pid, &process ) != zenith::status::successful_operation )
			return zenith::status::failed_sanity_check;
